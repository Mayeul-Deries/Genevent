...
//-----  Éléments du dialogue  -------------------------------------------------
    ...
    private enum Commande {
        ...
        AFFICHER_VOS_EVENEMENTS(3, "Afficher les évènement que vous avez créés")
        ;
        ...
        Commande(int code, String description) {
            assert code >= 0;
            this.code = code;
...
//-----  Éléments du dialogue  -------------------------------------------------
    ...
    private InfosNouvelEvenement dialogueSaisirNouvelEvenement(final Set<String> nomsExistants) {
        ...
        return new InfosNouvelEvenement(nom, dateDebut, dateFin, admin, lieu);
    }
        ...
        do {
            ...
            switch (cmd) {
                ...
                case CREER_EVENEMENT:
                    ...
                    break;
//                case AFFICHER_VOS_EVENEMENTS:
//                    this.controleur.afficherVosEvenements();
//                    break;
                default:
...
//-----  Primitives de lecture  ------------------------------------------------
    ...
    /**
     * Lit sur l'entrée standard un lieu
     *
     * @return Le lieu saisi par l'utilisa·teur/trice.
     */
    private static String lireLieu(final Set<String> lieux, final boolean nouveau) {
        return CLI.lireAvecErreurs(token -> Optional.of(token));
    }
    ...
    private static Optional<LocalDate> parseDate(final String token, final Optional<LocalDate> apres) {
        ...
        try {
            ...
            if (apres.isPresent() && apres.get().isAfter(date)) {
                ...
                CLI.afficher("La date saisie n'est pas ultérieure à " + apres.get());
                result = Optional.empty();
            } else {
    ...
    private static LocalDate lireDate(final Optional<LocalDate> apres) {
        ...
        apres.ifPresent(date -> CLI.afficher("La date doit être ultérieure à " + date));
     ...
     * Lit un lieu
     *
     * @return Le lieu saisie par l'utilisa·teur/trice.
     */
    private static String lireLieu() {
        return CLI.lireLieu(Collections.emptySet(), true);
    }
}
